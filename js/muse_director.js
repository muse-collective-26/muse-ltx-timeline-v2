...
const BG_AUDIO_TRACK_HEIGHT = 80;
const MOTION_TRACK_HEIGHT = 80; // used as Motion Guide track height
const CANVAS_HEIGHT = RULER_HEIGHT + BLOCK_HEIGHT + MOTION_TRACK_HEIGHT + AUDIO_TRACK_HEIGHT + BG_AUDIO_TRACK_HEIGHT;
...
function parseInitial(jsonStr) {
  ...
  try {
    if (jsonStr) {
      ...
      if (Array.isArray(p.bgAudioSegments)) {
        parsed.bgAudioSegments = p.bgAudioSegments.map(s => {
          const { _sSecs, _lSecs, _tSecs, _dSecs, _uploading, _decoding, ...rest } = s;
          return rest;
        });
      }
      if (p.bgAudioVolume !== undefined) parsed.bgAudioVolume = p.bgAudioVolume;
    }
  } catch (e) { }
  ...
  if (!parsed.bgAudioSegments) parsed.bgAudioSegments = [];
  for (let seg of parsed.bgAudioSegments) {
    if (!seg.id) {
      seg.id = Date.now().toString() + Math.random().toString(36).substr(2, 5);
    }
    if (seg.trimStart === undefined) seg.trimStart = 0;
...
class TimelineEditor {
  constructor(node, container, domWidget) {
    ...
    this.bgAudioTrackHeight = BG_AUDIO_TRACK_HEIGHT;
    this.canvasHeight = CANVAS_HEIGHT;

    // Core data
    this.timeline = { segments: [], motionSegments: [], audioSegments: [], bgAudioSegments: [], bgAudioVolume: 1.0 };
    ...
    console.log("[MuseTimeline] Constructor: timelineDataWidget value:", this.timelineDataWidget?.value);
    ...
    console.log("[MuseTimeline] Constructor: parsed timeline:", JSON.stringify(this.timeline));
  ...
  updateSelectionFromBox() {
    ...
    for (const track of ["image", "audio", "bg_audio", "motion"]) {
      ...
      } else if (track === "bg_audio") {
        trackTop = RULER_HEIGHT + this.blockHeight + this.audioTrackHeight;
        trackBottom = RULER_HEIGHT + this.blockHeight + this.audioTrackHeight + this.bgAudioTrackHeight;
      } else if (track === "motion") {
        trackTop = RULER_HEIGHT + this.blockHeight + this.audioTrackHeight + this.bgAudioTrackHeight;
        trackBottom = RULER_HEIGHT + this.blockHeight + this.audioTrackHeight + this.bgAudioTrackHeight + this.motionTrackHeight;
      }
  ...
  getSegmentArray(trackType) {
    ...
    if (trackType === "bg_audio") return this.timeline.bgAudioSegments || (this.timeline.bgAudioSegments = []);
  ...
  getTrackFromY(y) {
    if (y > RULER_HEIGHT + this.blockHeight + this.audioTrackHeight + this.bgAudioTrackHeight) return "motion";
    if (y > RULER_HEIGHT + this.blockHeight + this.audioTrackHeight) return "bg_audio";
  ...
  async _getOrExtractAudio(seg) {
    ...
    if (this._audioExtractionPromises.has(fileKey)) {
      try {
        ...
        console.warn("[MuseTimeline] Awaiting shared server audio extract promise failed:", err);
      }
      return;
    }
    ...
    try {
      ...
      console.warn("[MuseTimeline] Server audio check/extract failed:", err);
    } finally {
  ...
  createDOM() {
    ...
    // ── Muse Collective toggles (wired to ComfyUI node widgets) ──────────────
    const museToggleGroup = document.createElement("div");
    museToggleGroup.style.cssText = "display:flex;gap:4px;align-items:center;margin-left:6px;";

    const makeMuseToggle = (label, widgetName, color) => {
      const btn = document.createElement("button");
      btn.className = "pr-btn";
      btn.style.cssText = `padding:4px 8px;font-size:10px;font-weight:600;height:28px;box-sizing:border-box;border-radius:4px;cursor:pointer;transition:background 0.15s;`;
      const sync = () => {
        const w = this.node.widgets?.find(w => w.name === widgetName);
        const on = w ? !!w.value : false;
        btn.textContent = label;
        btn.style.background = on ? color : "#2a2a2a";
        btn.style.color = on ? "#fff" : "#888";
        btn.style.borderColor = on ? color : "#444";
      };
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        const w = this.node.widgets?.find(w => w.name === widgetName);
        if (w) { w.value = !w.value; if (w.callback) w.callback(w.value); }
        sync();
      });
      sync();
      museToggleGroup.appendChild(btn);
      return { btn, sync };
    };
    ...
    settingsToggleBtn.style.cssText = "padding:4px 8px;font-size:10px;font-weight:600;height:28px;box-sizing:border-box;border-radius:4px;cursor:pointer;margin-left:6px;";
    let _museSettingsVisible = false;
    ...
    settingsToggleBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      _museSettingsVisible = !_museSettingsVisible;
      for (const w of this.node.widgets || []) {
        if (MUSE_SETTINGS_WIDGET_NAMES.includes(w.name)) {
          _museSettingsVisible ? showWidget(w) : hideWidget(w);
        }
      }
      updateSettingsToggleStyle();
    ...
    this.wrapper.addEventListener("drop", (e) => {
      ...
      if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
        ...
        } else if (audioFiles.length > 0 && targetTrack === "bg_audio") {
          this.handleBgAudioUpload(audioFiles, targetFrameStart);
    ...
    volIcon.style.cssText = "font-size:10px;color:#aac;";
    ...
    volSlider.addEventListener("input", (e) => {
      ...
      this.timeline.bgAudioVolume = v;
      volDisplay.textContent = v.toFixed(2);

      // Also update bg_volume widget if present (for V3 node)
      const bgVolWidget = this.node.widgets?.find(w => w.name === "bg_volume");
      if (bgVolWidget) bgVolWidget.value = v;
    ...
    this.sidebar.appendChild(this.bgAudioTrackLabel);
  ...
  syncWidgetsAndUI() {
    console.log("[MuseTimeline] syncWidgetsAndUI() called.");
    ...
    try {
      ...
      if (checkResp.status === 200) {
        ...
        if (checkResult.exists) {
          console.log(`[MuseTimeline] File already exists: ${checkResult.name}. Reusing existing file.`);
          return checkResult.name;
        }
      }
    } catch (e) {
      console.warn("[MuseTimeline] Failed to check for existing file, proceeding with upload", e);
    }
        ...
        try {
          ...
          vid.onloadeddata = async () => {
            ...
            vid.onseeked = () => {
              ...
              if (IS_LARGE_FILE) {
                console.log(`[MuseTimeline] Large file detected (${(file.size / 1024 / 1024).toFixed(1)} MB). Offloading audio extraction to server.`);
              } else {
              ...
              this._uploadVideoFile(file).then(filePath => {
                  ...
                  api.fetchApi(`/ltx_director_get_audio?filename=${encodeURIComponent(filePath)}`)
                    ...
                    .then(res => {
                      if (res.audio_file && res.peaks) {
                        ...
                        // Fallback
                        if (IS_LARGE_FILE) {
                          console.warn("[MuseTimeline] Server audio extraction failed for large file, skipping.");
                          for (let s of this.timeline.audioSegments) {
                            if (s.audioFile === filePath || s._blobUrl === blobUrl) {
                              s._decoding = false;
                            }
                    ...
                    .catch(err => {
                      console.error("[MuseTimeline] Server audio extraction query failed:", err);
                      for (let s of this.timeline.audioSegments) {
                        if (s.audioFile === filePath || s._blobUrl === blobUrl) {
                          s._decoding = false;
                        }
                ...
                console.error("[MuseTimeline] Background video upload failed", err);
    ...
    for (let file of files) {
      ...
      await new Promise(async (resolve) => {
        try {
          ...
          vid.onloadeddata = () => {
            ...
            vid.onseeked = () => {
              ...
              this._uploadVideoFile(file).then(filePath => {
                ...
                console.error("[MuseTimeline] Background motion video upload failed", err);
          ...
          console.error("[MuseTimeline] Motion video processing failed", err);
          resolve();
        }
      });
    }
  ...
  async handleBgAudioUpload(files, targetFrameStart = null) {
    const frameRate = this.getFrameRate();
    if (!this.timeline.bgAudioSegments) this.timeline.bgAudioSegments = [];
    ...
    for (let file of files) {
      ...
      await new Promise(async (resolve) => {
        try {
          ...
          this.timeline.bgAudioSegments.push(seg);
          this.timeline.bgAudioSegments.sort((a, b) => a.start - b.start);

          this.selectionType = "bg_audio";
          this.selectedIndex = this.timeline.bgAudioSegments.findIndex(s => s.id === seg.id);

          this.commitChanges(true);
          this.render();
          resolve();
        } catch (err) {
          console.error("[MuseTimeline] BG audio processing failed", err);
          resolve();
        }
      });
    }
  ...
  deleteSelectedSegment() {
    if (this.selectedSegmentIds && this.isMultiSelectActive()) {
      ...
      if (this.timeline.bgAudioSegments) this.timeline.bgAudioSegments = this.timeline.bgAudioSegments.filter(s => !idsToDelete.has(s.id));
      ...
      if (this.selectionType === "bg_audio") {
        if (!this.timeline.bgAudioSegments || this.timeline.bgAudioSegments.length === 0 || this.selectedIndex === -1) return;
        this.timeline.bgAudioSegments.splice(this.selectedIndex, 1);
  ...
  getCanonicalTrack(track) {
    ...
    if (track === "bg_audio") return "bg_audio";
  ...
  updateRetakeUIState() {
    ...
    if (isRetake) {
      ...
      if (this.mainTrackLabel) {
        ...
        if (this.bgAudioTrackLabel) this.bgAudioTrackLabel.style.display = "none";
      ...
      if (this.mainTrackLabel) {
        ...
        if (this.bgAudioTrackLabel) this.bgAudioTrackLabel.style.display = "flex";
  ...
  updateSidebarHeights() {
    if (this.mainTrackLabel) {
      ...
      if (this.bgAudioTrackLabel) this.bgAudioTrackLabel.style.height = `${this.bgAudioTrackHeight}px`;
  ...
  render() {
    ...
    let renderBgAudioSegments = this.timeline.bgAudioSegments || [];
    ...
    const sortedBgAudioSegments = [...renderBgAudioSegments].sort((a, b) => {
      const aSel = this.selectedSegmentIds.includes(a.id) ? 1 : 0;
      const bSel = this.selectedSegmentIds.includes(b.id) ? 1 : 0;
      return aSel - bSel;
    });
    ...
    if (this.retakeMode) {
      ...
      // --- Draw BG Audio Segments ---
      for (let i = 0; i < sortedBgAudioSegments.length; i++) {
        const seg = sortedBgAudioSegments[i];
        const rawStartX = (seg.start / totalFrames) * width;
        const rawEndX = ((seg.start + seg.length) / totalFrames) * width;
        const startX = Math.floor(rawStartX);
        const pxWidth = Math.max(1, Math.floor(rawEndX) - startX);
        const isSelected = this.selectedSegmentIds.includes(seg.id);
        const bgTrackY = RULER_HEIGHT + this.blockHeight + this.audioTrackHeight;

        if ((this._isDragging && this.selectionType === "bg_audio" && seg.id === this._dragTargetId) || (this._ghostSegmentId && seg.id === this._ghostSegmentId)) {
          this.ctx.globalAlpha = 0.65;
        } else {
          this.ctx.globalAlpha = 1.0;
        }
        ...
        if (seg.type === "ghost") {
          this.ctx.fillStyle = "#1a1a1a";
          this.ctx.fillRect(startX, bgTrackY, pxWidth, this.bgAudioTrackHeight);
          this.ctx.strokeStyle = "#555";
          this.ctx.lineWidth = 2;
          this.ctx.setLineDash([5, 5]);
          this.ctx.strokeRect(startX, bgTrackY, pxWidth, this.bgAudioTrackHeight);
          this.ctx.setLineDash([]);
          this.ctx.fillStyle = "#888";
          this.ctx.textAlign = "center";
          this.ctx.textBaseline = "middle";
          this.ctx.font = "bold 12px sans-serif";
          this.ctx.fillText("Drop BG Audio", startX + pxWidth / 2, bgTrackY + this.bgAudioTrackHeight / 2);
        } else {
          const showHandles = !this.isMultiSelectActive();
          const outlineColor = isSelected ? "#fff" : null;
          ...
          if (seg.waveformPeaks && pxWidth > 0) {
            ...
            const centerY = bgTrackY + this.bgAudioTrackHeight / 2;
            for (let px = 0; px < pxWidth; px++) {
              const pixelRatio = px / pxWidth;
              ...
              if (peakIdx >= 0 && peakIdx < peakCount) {
                ...
                const amp = (val * (this.bgAudioTrackHeight - 12) / 2) * 0.9;
                this.ctx.fillRect(startX + px, centerY - amp, 1, amp * 2);
              }
            }
          }
          ...
          if (isSelected && showHandles) {
            ...
            this.ctx.beginPath();
            this.ctx.roundRect(startX, bgTrackY + this.bgAudioTrackHeight / 2 - 12, 4, 24, 2);
            this.ctx.fill();
            this.ctx.beginPath();
            this.ctx.roundRect(startX + pxWidth - 4, bgTrackY + this.bgAudioTrackHeight / 2 - 12, 4, 24, 2);
            this.ctx.fill();
          }
          // Filename label
          ...
          this.ctx.save();
          this.ctx.beginPath();
          this.ctx.rect(startX, bgTrackY + 2, pxWidth, this.bgAudioTrackHeight - 3);
          this.ctx.clip();
          let text = seg.fileName || "BG Audio";
          const maxWidth = pxWidth - 12;
          if (this.ctx.measureText(text).width > maxWidth && maxWidth > 0) {
            while (text.length > 0 && this.ctx.measureText(text + "...").width > maxWidth) {
              text = text.slice(0, -1);
            }
            text = text + "...";
          }
          this.ctx.fillText(text, startX + 6, bgTrackY + 8);
          // Volume indicator
          if (pxWidth > 60) {
            const vol = this.timeline.bgAudioVolume !== undefined ? this.timeline.bgAudioVolume : 1.0;
            const volText = `Vol: ${vol.toFixed(2)}`;
            this.ctx.font = "9px sans-serif";
            this.ctx.fillStyle = "rgba(170, 200, 255, 0.7)";
            this.ctx.fillText(volText, startX + 6, bgTrackY + this.bgAudioTrackHeight - 16);
          }
          this.ctx.restore();
          this.ctx.restore();
        }
    ...
    if (y >= RULER_HEIGHT && y <= this.canvasHeight) {
      ...
      for (let i = 0; i < gapRegions.length; i++) {
        ...
        if (dx * dx + dy2 * dy2 <= BTN_R * BTN_R) {
          ...
          } else if (currentTrack === "bg_audio" && !isCompatible) {
            this.promptAddBgAudioInGap(gap.frameStart, gap.frameEnd);
          } else {
  ...
  commitChanges(skipRender = false) {
    ...
    const toSave = {
      ...
      bgAudioSegments: (this.timeline.bgAudioSegments || []).map(s => {
        const { _sSecs, _lSecs, _tSecs, _dSecs, _uploading, _decoding, _blobUrl, _audioBuffer, ...rest } = s;
        return rest;
      }),
      bgAudioVolume: this.timeline.bgAudioVolume !== undefined ? this.timeline.bgAudioVolume : 1.0,
    };

    const jsonStr = JSON.stringify(toSave);
    console.log("[MuseTimeline] commitChanges: saving timelineDataWidget value:", jsonStr);
  ...
  getGapRegions() {
    ...
    for (const seg of sortedMot) {
      if (seg.start > cursor) {
        const x0 = (cursor / totalFrames) * width;
        const x1 = (seg.start / totalFrames) * width;
        gaps.push({ track: 'motion', frameStart: cursor, frameEnd: seg.start, centerX: (x0 + x1) / 2, centerY: RULER_HEIGHT + this.blockHeight + this.audioTrackHeight + this.bgAudioTrackHeight + this.motionTrackHeight / 2, widthPx: x1 - x0 });
      }
      cursor = seg.start + seg.length;
    }
    if (cursor < outputFrames) {
      const x0 = (cursor / totalFrames) * width;
      const x1 = (outputFrames / totalFrames) * width;
      gaps.push({ track: 'motion', frameStart: cursor, frameEnd: outputFrames, centerX: (x0 + x1) / 2, centerY: RULER_HEIGHT + this.blockHeight + this.audioTrackHeight + this.bgAudioTrackHeight + this.motionTrackHeight / 2, widthPx: x1 - x0 });
    }
    ...
    // BG Audio gaps
    cursor = 0;
    const bgAudioY = RULER_HEIGHT + this.blockHeight + this.audioTrackHeight + this.bgAudioTrackHeight / 2;
    const sortedBgAud = [...(this.timeline.bgAudioSegments || [])].sort((a, b) => a.start - b.start);
    for (const seg of sortedBgAud) {
      if (seg.start > cursor) {
        const x0 = (cursor / totalFrames) * width;
        const x1 = (seg.start / totalFrames) * width;
        gaps.push({ track: 'bg_audio', frameStart: cursor, frameEnd: seg.start, centerX: (x0 + x1) / 2, centerY: bgAudioY, widthPx: x1 - x0 });
      }
      cursor = seg.start + seg.length;
    }
    if (cursor < outputFrames) {
      const x0 = (cursor / totalFrames) * width;
      const x1 = (outputFrames / totalFrames) * width;
      gaps.push({ track: 'bg_audio', frameStart: cursor, frameEnd: outputFrames, centerX: (x0 + x1) / 2, centerY: bgAudioY, widthPx: x1 - x0 });
    }
  ...
  promptAddBgAudioInGap(frameStart, frameEnd) {
    const fi = document.createElement("input");
    fi.type = "file";
    fi.accept = "audio/*";
    fi.addEventListener("change", (ev) => {
      if (ev.target.files?.[0]) this.handleBgAudioUpload([ev.target.files[0]], frameStart);
    });
    fi.click();
  }
  ...
  onContextMenu(e) {
    ...
    const isBgAudioTrack = mouseY >= RULER_HEIGHT + trackHeight + this.audioTrackHeight && mouseY <= RULER_HEIGHT + trackHeight + this.audioTrackHeight + this.bgAudioTrackHeight;
    const isMotionTrack = mouseY >= RULER_HEIGHT + trackHeight + this.audioTrackHeight + this.bgAudioTrackHeight && mouseY <= RULER_HEIGHT + trackHeight + this.audioTrackHeight + this.bgAudioTrackHeight + this.motionTrackHeight;
    ...
    if (isMotionTrack) {
      ...
      trackType = "motion";
    } else if (isBgAudioTrack) {
      clickedSeg = (this.timeline.bgAudioSegments || []).find(s => cursor >= s.start && cursor <= s.start + s.length);
      trackType = "bg_audio";
    ...
    } else if (isMotionTrack || isImageTrack || isAudioTrack || isBgAudioTrack) {
      const gapRegions = this.getGapRegions();
      const currentTrack = isMotionTrack ? "motion" : (isBgAudioTrack ? "bg_audio" : (isAudioTrack ? "audio" : "image"));
  ...
  showGapContextMenu(clientX, clientY, gap) {
    ...
    } else if (currentTrack === "bg_audio") {
      const bgAudBtn = document.createElement("button");
      bgAudBtn.className = "pr-gap-menu-btn";
      bgAudBtn.innerHTML = `${ICONS.audio} BG Audio Segment`;
      bgAudBtn.onclick = () => {
        this.dismissContextMenu();
        this.promptAddBgAudioInGap(gap.frameStart, gap.frameEnd);
      };
      menu.appendChild(bgAudBtn);
    }
  ...
  showGapMenu(clientX, clientY, gap) {
    ...
    if (currentTrack === "image") {
      ...
      bgAudBtn.className = "pr-gap-menu-btn";
      bgAudBtn.innerHTML = `${ICONS.audio} BG Audio Segment`;
      bgAudBtn.addEventListener("click", () => {
        this.dismissGapMenu();
        this.promptAddBgAudioInGap(gap.frameStart, gap.frameEnd);
      });
      menu.appendChild(bgAudBtn);
...
app.registerExtension({
  name: "MuseCollective.LTXTimeline",
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "MuseDirectorSamplerV1") {
      ...
      nodeType.prototype.onConfigure = function (info) {
        ...
        console.log("[MuseTimeline] onConfigure called. info.widgets_values:", info.widgets_values ? JSON.stringify(info.widgets_values) : "undefined");
        ...
        if (info.properties && info.properties.has_serialized_properties) {
          console.log("[MuseTimeline] Restoring widgets from properties");
          if (this.widgets) {
            for (const w of this.widgets) {
        ...
        setTimeout(() => {
          if (this._timelineEditor) {
            console.log("[MuseTimeline] setTimeout sync block called.");
            console.log("[MuseTimeline] setTimeout: timelineDataWidget value:", this._timelineEditor.timelineDataWidget?.value);
            const tl = parseInitial(this._timelineEditor.timelineDataWidget?.value);
            console.log("[MuseTimeline] setTimeout: parsed timeline:", JSON.stringify(tl));
